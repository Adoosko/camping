import Image from "next/image";
import Button from "./Button";

const Hero = () => {
  return (
    <section
      className=" max-container padding-container bg-neutral-900
     flex flex-col gap-20 py-10 pb-32 md:gap-28 lg:py-20 xl:flex-row"
    >
      <div />

      {/* LEFT SIDE */}
      <div className="relative z-20 flex flex-1 flex-col xl:w-1/2">
        <Image
          src={"/food.png"}
          alt="camp"
          width={50}
          height={50}
          className="absolute left-[-5px] top-[-30px] w-10 lg:w-[40px]"
        ></Image>
        <h1 className="bold-52 text-white lg:bold-88">Na Formu</h1>
        <p className="regular-24 mt-6 text-gray-10 xl:max-w-[520px]">
          „Tvoja cesta k forme začína na tanieri.“
        </p>
        <div className="my-11 flex flex-wrap gap-5">
          <div className="flex items-center gap-2">
            {Array(5)
              .fill(1)
              .map((_, index) => (
                <Image
                  src={"/star.svg"}
                  key={index}
                  alt="star"
                  width={24}
                  height={24}
                ></Image>
              ))}
          </div>
          <p className="bold-16 lg:bold-20 text-white">
            Neuveriteľné
            <span className="regular-16 lg:regular-20 ml-1 underline">
              výsledky
            </span>
          </p>
        </div>
        <div className="flex flex-col w-full gap-3 sm:flex-row">
          <Button
            type="button"
            icon="muscle.svg"
            title="Nabrať"
            variant="btn_green"
          />
          <Button type="button" title="Schudnúť" variant="btn_white_text" />
        </div>
      </div>
      <div className="relative flex flex-1 items-start ">
        <div
          className="relative z-20 w-[268px] flex flex-col gap-8
         rounded-3xl bg-green-90 px-7 py-8"
        >
          <div className="flex flex-col">
            <div className="flexBetween">
              <p className="regular-16 text-gray-20">Dĺžka obdobia</p>
            </div>
            <p className="bold-20 text-white">4 mesiace</p>
          </div>

          <div className="flexBetween">
            <div className="flex flex-col">
              <p className="regular-16 block text-gray-20">Počiatočná váha</p>
              <p className="bold-20 text-white">54 KG</p>
            </div>
            <div className="flex flex-col">
              <p className="regular-16 block text-gray-20">Aktuálna váha</p>
              <p className="bold-20 text-white">74.2 KG</p>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Hero;
